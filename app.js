...
document.getElementById('downloadBtn').addEventListener('click', async () => {
    try {
        ...
        alert('✅ Descarga iniciada. Ahora puede proceder a la evaluación.');
    } catch (error) {
...
// Botón iniciar trivia
document.getElementById('iniciarTrivia').addEventListener('click', () => {
    if (!descargaRealizada) {
        alert('⚠️ Debe descargar el tríptico antes de iniciar la evaluación.');
        return;
    }
...
function calcularPuntaje() {
    ...
    if (userData.aprobado) {
        ...
        statusText.innerHTML = '🎉 <strong style="color: #0066CC;">¡APROBADO!</strong>';
